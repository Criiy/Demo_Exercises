...
$staff = array
 ...
 function search(){
 	...
 	if (!isset($_GET["number"]) || empty($_GET["number"])) {
 		echo '{"success":false,"msg":"No info:("}';
 		return;
 	}
 	...
 	//获取number参数
 	$number = $_GET["number"];
 	$result = '{"success":false,"msg":"Staff not found:("}';

 	//遍历$staff多维数组，查找key值为number的员工是否存在，如果存在，则修改返回结果
 	foreach ($staff as $value) {
 		if ($value["number"] == $number) {
 			$result = '{ "success":true,"msg":"Result： Staff number' . $value["number"] .'<br>'.
                        'Staff name：' . $value["name"] .'<br>' .
                        'Staff gender：' . $value["sex"] .'<br>' .
                        'Staff job：' . $value["job"] . '"}';
 			break;
 		}
 	}
 ...
 function create(){
 	...
 	if (!isset($_POST["name"]) || empty($_POST["name"])
 		...
 		echo '{"success":false,"msg":"Info Incomplete:("}';
 		return;
 	}
 	//TODO: 获取POST表单数据并保存到数据库

 	//提示保存成功
 	echo '{"success":true,"msg":"Staff：' . $_POST["name"] . ' Save done！"}';
 }
